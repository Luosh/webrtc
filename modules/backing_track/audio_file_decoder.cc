...
AudioFileDecoder::AudioFileDecoder(TaskQueueFactory* task_queue_factory,
    ...
    if (stream_no_ < 0) {
        RTC_LOG(LS_ERROR) << "AudioFileDecoder:: av_find_best_stream fail "
                          << av_err2str(stream_no_);
        return;
    }

    if (!(codec = avcodec_find_decoder(
              format_context_->streams[stream_no_]->codecpar->codec_id))) {
        RTC_LOG(LS_ERROR) << "AudioFileDecoder:: avcodec_find_decoder fail";
        return;
    }
    ...
    fifo_.reset(av_audio_fifo_alloc(codec_context_->sample_fmt,
                                    codec_context_->channels, fifo_capacity_));
    if (!fifo_) {
...
int32_t AudioFileDecoder::channel_num() {
    return codec_context_ ? codec_context_->channels : 0;
}
